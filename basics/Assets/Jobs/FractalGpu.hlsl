...
#if defined(UNITY_PROCEDURAL_INSTANCING_ENABLED)
	float4 color;
	float interpolator = frac(unity_InstanceID * _SequenceNumbers.x + _SequenceNumbers.y);
	color.rgb = lerp(_ColorA.rgb, _ColorB.rgb, interpolator);
	color.a = lerp(_ColorA.a, _ColorB.a, interpolator);
	return color;
#else
	return _ColorA;
#endif
